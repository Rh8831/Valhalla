...
def _fetch_client_traffic(panel_url: str, token: str, client_id: str) -> Optional[int]:
    """Return ``up+down`` traffic for a client via fallback endpoint.

    Some 3x-ui deployments do not include ``clientStats`` inside the
    ``inbounds/list`` response.  When statistics are missing we query the
    dedicated ``getClientTrafficsById`` endpoint to obtain usage for the
    given client UUID.
    """
    try:
        r = requests.get(
        ...
        pass  # pragma: no cover - network errors
    return None
    ...
    for inbound, client in matches:
        ...
        for st in inbound.get('clientStats', []) or []:
            if st.get('id') == uuid:
                ...
                break
        if not got and uuid:
            extra = _fetch_client_traffic(panel_url, token, uuid)
